...
class MaxProbExtractor(nn.Module):
    ...
    def forward(self, YOLOoutput):
        ...
        max_conf_target, max_conf_idx_target = torch.max(confs_if_object, dim=1)  # batch, batch. torch.Size([8]), torch.Size([8])
...
class CSSCalculator(nn.Module):
    ...
    def forward(self, adv_patch, sample_img):
        ...
        if((n_items_adv_patch == 3)and(n_items_sample_img==3)):
            ...
            adv_patch = F.interpolate(adv_patch.unsqueeze(0), (h_target_size, w_target_size))[0]     # torch.Size([3, 75, 75])
            self.color_array.data = F.interpolate(self.color_array, (h_target_size, w_target_size))  # torch.Size([1, 3, 75, 75])
            ...
            # only work with the min distance
            color_dist_prod = torch.min(color_dist, 0)[0] #test: change prod for min (find distance to closest color)  ##  torch.Size([75, 75])
            # calculate the nps by summing over all pixels
            nps_score = torch.sum(color_dist_prod,0)  ##                                                                   torch.Size([75])
            nps_score = torch.sum(nps_score,0)  ##                                                                         torch.Size([])
            return nps_score/torch.numel(adv_patch)
            ...
            color_dist_prod = torch.min(color_dist, 0)[0] #test: change prod for min (find distance to closest color)  ##  torch.Size([75, 75])
            # calculate the nps by summing over all pixels
            nps_score = torch.sum(color_dist_prod,1)  ##                                                                   torch.Size([75])
            nps_score = torch.sum(nps_score,1)  ##                                                                         torch.Size([])
...
class PatchSimpleTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False):
        if(with_crease):
            # warping
        ...
        adv_batch_masked, msk_batch = resize_rotate(adv_batch)  # adv_batch torch.Size([8, 7, 3, 150, 150])   adv_batch_masked torch.Size([8, 7, 3, 416, 416]) 
...
class PatchTransformer(nn.Module):
    ...
    def forward(self, adv_patch, lab_batch, img_size, patch_mask=[], by_rectangle=False, do_rotate=True, rand_loc=True, with_black_trans=False, scale_rate=0.2, with_crease=False, with_projection=False, with_rectOccluding=False, enable_empty_patch=False, enable_no_random=False, enable_blurred=True):
        ...
        # adv_batch_masked, adv_batch_masked0, adv_batch_masked1, adv_batch_masked3, adv_batch_masked4, adv_batch_t, msk_batch_t = resize_rotate(adv_batch)
        adv_batch_masked, msk_batch = resize_rotate(adv_batch, by_rectangle)  # adv_batch torch.Size([8, 7, 3, 150, 150])   adv_batch_masked torch.Size([8, 7, 3, 416, 416]) 
...
if __name__ == '__main__':
    raise SystemExit(
        'The standalone legacy Darknet demo was removed from this trimmed bundle. '
        'Use the modern_yolo_attack entrypoints instead.'
    )
