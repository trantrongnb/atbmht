...
from GANLatentDiscovery.models.BigGAN import BigGAN, utils
from GANLatentDiscovery.models.gan_with_shift import gan_with_shift
from ipdb import set_trace as st


# try:
#     from GANLatentDiscovery.models.StyleGAN2.model.notyet import Generator as StyleGAN2Generator
...
class ConditionedBigGAN(nn.Module):
    ...
    def mixed_classes(self, batch_size):
        if len(self.target_classes.data.shape) == 0:
            return self.target_classes.repeat(batch_size).to(self.target_classes.device)
        else:
            return torch.from_numpy(
                np.random.choice(self.target_classes.cpu(), [batch_size])).to(self.target_classes.device)
...
def make_biggan_config(weights_root):
    with open('GANLatentDiscovery/models/BigGAN/generator_config.json') as f:
        config = json.load(f)
...
def make_big_gan(weights_root, target_class):
    ...
    return ConditionedBigGAN(G, target_class)
